<%@ Page Title="" Language="C#" MasterPageFile="~/afterLoginMaster.master" AutoEventWireup="true" CodeFile="editEmployee3.aspx.cs" Inherits="editEmployee3" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    <asp:ScriptManager ID="ScriptManager1" runat="server">
    </asp:ScriptManager>
    <asp:UpdatePanel ID="UpdatePanel1" runat="server">
        <ContentTemplate>
            <table border="1" cellpadding="2" cellspacing="1" class="style1">
                <tr>
                    <td colspan="2" 
                        style="text-align: center; font-style: italic; font-size: large">
                        Update Employee Details</td>
                </tr>
                <tr>
                    <td style="text-align: center" width="50%">
                        Id</td>
                    <td style="text-align: center" width="50%">
                        <asp:TextBox ID="TextBox1" runat="server" ReadOnly="True" Width="60%"></asp:TextBox>
                    </td>
                </tr>
                <tr>
                    <td style="text-align: center" width="50%">
                        Name</td>
                    <td style="text-align: center" width="50%">
                        <asp:TextBox ID="TextBox2" runat="server" Width="60%"></asp:TextBox>
                    </td>
                </tr>
                <tr>
                    <td style="text-align: center" width="50%">
                        Department</td>
                    <td style="text-align: center" width="50%">
                        <asp:TextBox ID="TextBox3" runat="server" Width="60%"></asp:TextBox>
                    </td>
                </tr>
                <tr>
                    <td style="text-align: center" width="50%">
                        Salary</td>
                    <td style="text-align: center" width="50%">
                        <asp:TextBox ID="TextBox4" runat="server" Width="60%"></asp:TextBox>
                    </td>
                </tr>
                <tr>
                    <td style="text-align: center" width="50%">
                        City</td>
                    <td style="text-align: center" width="50%">
                        <asp:TextBox ID="TextBox5" runat="server" Width="60%"></asp:TextBox>
                    </td>
                </tr>
                <tr>
                    <td style="text-align: center" width="50%">
                        Employee User Role</td>
                    <td style="text-align: center" width="50%">
                        <asp:RadioButtonList ID="RadioButtonList1" runat="server" RepeatColumns="2" 
                            Width="100%">
                            <asp:ListItem>Employee</asp:ListItem>
                            <asp:ListItem>Admin</asp:ListItem>
                        </asp:RadioButtonList>
                    </td>
                </tr>
                <tr>
                    <td colspan="2" style="text-align: center">
                        <asp:Button ID="Button1" runat="server" onclick="Button1_Click" Text="Update" 
                            Width="60%" />
                    </td>
                </tr>
            </table>
        </ContentTemplate>
    </asp:UpdatePanel>
</asp:Content>

